 ...
 * @version #{VERSION}
 * @see http://github.com/lambdalisue/rhoallet/
 * @copyright 2014, Alisue, hashnote.net
 *
 */
...
exec = (file, args, done) ->
  ...
  return proc
execAndWrite = (file, args, out, done) ->
  proc = spawn(file, args)
  proc.stdout.on 'data', (data) -> fs.writeFileSync(out, data)
  proc.stderr.on 'data', (data) -> process.stderr.write(data)
  proc.on('exit', (code) -> done(code)) if done
  return proc
...
compileMustache = (options, done) ->
  proc = spawn(HULK, ['tmpl/*.mustache'])
  proc.stdout.on 'data', (data) -> fs.writeFileSync('www/js/templates.js', "templates = {};\n" + data)
  proc.stderr.on 'data', (data) -> process.stderr.write(data)
  proc.on('exit', (code) -> done(code)) if done
task 'compile:mustache', 'compile Mustache files', (options) ->
  compileMustache(options)
